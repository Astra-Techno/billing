...
const allMenus = [
  {
    ...
    items: [
      ...
      { path: '/purchase-orders', label: 'Orders', icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01', feature: 'purchase_orders', permission: 'purchase_orders' },
      { path: '/credit-notes', label: 'Credits', icon: 'M16 15v-1a4 4 0 00-4-4H8m0 0l3 3m-3-3l3-3m9 14V5a2 2 0 00-2-2H6a2 2 0 00-2 2v16l4-2 4 2 4-2 4 2z', feature: 'credit_notes', permission: 'credit_notes' },
    ]
  },
  {
    group: 'People',
    items: [
      { path: '/clients', label: 'Clients', icon: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z', permission: 'clients' },
    ]
  },
  {
    group: 'Finance',
    items: [
      { path: '/expenses', label: 'Expenses', icon: 'M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z', feature: 'expenses', permission: 'expenses' },
      { path: '/payroll',  label: 'Payroll',  icon: 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z', feature: 'payroll', permission: 'payroll' },
      { path: '/products', label: 'Products', icon: 'M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4', permission: 'products' },
      { path: '/timesheets', label: 'Timesheets', icon: 'M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z', permission: 'timesheets' },
    ]
  },
  {
    group: 'Reports',
    items: [
      { path: '/gst-returns', label: 'GST',     icon: 'M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z', feature: 'gst_returns', permission: 'gst' },
      { path: '/reports',    label: 'Reports',  icon: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z', feature: 'reports', permission: 'reports' },
    ]
  },
]
...
const groups = computed(() => {
  ...
  return allMenus.map(g => ({
    ...
    items: g.items.filter(m =>
      (!m.feature || biz.isEnabled(m.feature)) &&
...
<template>
  <aside class="hidden lg:flex flex-col shrink-0 z-40 relative overflow-y-auto no-scrollbar sidebar-root">
    ...
    <nav class="flex-1 px-3 py-3 overflow-y-auto no-scrollbar">
      ...
      <template v-if="auth.isSuperAdmin && !auth.businessId">
        ...
        <RouterLink to="/admin/desktop-licenses"
          class="nav-item"
          :class="route.path.startsWith('/admin/desktop-licenses') ? 'nav-active' : 'nav-default'">
          <svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" d="M9 12l2 2 4-4m5 2a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
          <span>Desktop Licences</span>
        </RouterLink>
      </template>
    ...
    <div class="sidebar-footer">
      <RouterLink v-if="(!auth.isSuperAdmin || auth.businessId) && can('settings')"
        to="/settings"
        class="nav-item"
