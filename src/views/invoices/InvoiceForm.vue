          ...
          <div class="lg:hidden space-y-4">
            ...
            <div>
              <div class="flex justify-between items-center mb-1.5 px-1">
                <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Invoice Details</p>
                <!-- Done button when editing, Pencil when collapsed -->
                <button type="button" @click="editingInvoiceDetails = !editingInvoiceDetails"
                  class="w-7 h-7 rounded-full flex items-center justify-center transition"
                  :class="editingInvoiceDetails ? 'bg-primary-600 text-white shadow' : 'bg-blue-50 text-[#1a56db] hover:bg-blue-100'">
                  <svg v-if="editingInvoiceDetails" class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
                  <svg v-else class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>
                </button>
              </div>

              <div class="inv-card p-4">
                <!-- 1. Collapsed state (Text grid matching Mockup screenshot) -->
                <div v-if="!editingInvoiceDetails" class="grid grid-cols-2 gap-x-4 gap-y-3.5 text-sm">
                  <div>
                    <p class="text-xs text-gray-400 font-semibold uppercase tracking-wider">Number</p>
                    <p class="font-bold text-gray-800 mt-1">#{{ invoiceNumber || 'Draft' }}</p>
                  </div>
                  <div>
                    <p class="text-xs text-gray-400 font-semibold uppercase tracking-wider">Bill Type</p>
                    <p class="font-bold text-gray-800 mt-1 truncate">
                      {{ form.invoice_type === 'tax_invoice' ? 'Tax Invoice (with GST)' : form.invoice_type === 'bill_of_supply' ? 'Bill of Supply' : 'Retail Invoice' }}
                    </p>
                  </div>
                  <div>
                    <p class="text-xs text-gray-400 font-semibold uppercase tracking-wider">Invoice Date</p>
                    <p class="font-bold text-gray-800 mt-1">{{ formatDateDisplay(form.issue_date) }}</p>
                  </div>
                  <div>
                    <p class="text-xs text-gray-400 font-semibold uppercase tracking-wider">Due Date</p>
                    <p class="font-bold text-gray-800 mt-1">{{ formatDateDisplay(form.due_date) }}</p>
                  </div>
                  <div class="col-span-2">
                    <p class="text-xs text-gray-400 font-semibold uppercase tracking-wider">Place of Supply</p>
                    <p class="font-bold text-gray-800 mt-1">{{ placeOfSupplyName || 'Tamil Nadu (Auto)' }}</p>
                  </div>
                </div>
                ...
                <div v-else class="space-y-4 animate-fade-in-up">
                  ...
                  <button type="button" @click="editingInvoiceDetails = false" class="w-full py-2 bg-primary-50 text-primary-700 text-xs font-bold rounded-xl hover:bg-primary-100 transition">
