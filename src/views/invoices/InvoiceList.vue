...
<template>
  <div class="flex flex-col lg:flex-row gap-6 h-[calc(100vh-6rem)]">
    <!-- Left Pane: List -->
    <div :class="{ 'hidden lg:flex': $route.name !== 'Invoices', 'w-full lg:w-[35%] flex flex-col': true }">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
      <div>
        ...
        <p class="text-sm text-gray-400 mt-0.5 font-medium">Track all your bills and payments</p>
      </div>
      <button @click="onNewBill" class="btn-primary">
    ...
    <div class="flex flex-col gap-3">
      ...
      <div v-if="showDatePicker" class="flex gap-2 items-center bg-white p-3 rounded-2xl shadow-soft animate-fade-in-up">
        <input v-model="filter.from_date" type="date" class="form-input text-sm flex-1 border-0 bg-gray-50"
          @change="filter.preset = ''; load()" />
        <span class="text-gray-400 text-sm font-medium">to</span>
        <input v-model="filter.to_date" type="date" class="form-input text-sm flex-1 border-0 bg-gray-50"
          @change="filter.preset = ''; load()" />
      </div>
    </div>
    ...
    <div class="bg-white rounded-[2rem] shadow-soft border-0 overflow-hidden animate-fade-in-up anim-delay-75">
          ...
          <div v-else-if="!invoices.length" class="p-12 text-center">
            ...
            <button @click="onNewBill" class="btn bg-primary-600 text-white hover:bg-primary-700 shadow-soft-blue rounded-full px-6 py-2.5 mt-5 inline-flex items-center gap-2 font-bold">
          ...
          <div v-else class="divide-y divide-gray-50">
            <div v-for="inv in invoices" :key="inv.id"
              ...
              <div class="w-12 h-12 rounded-full flex items-center justify-center shrink-0 font-extrabold text-lg" :class="avatarColor(inv.client_name)">
