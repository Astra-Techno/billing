...
async function approve(row){busy.value='r'+row.id;error.value='';try{await task('Admin','approveDesktopActivation',{request_id:row.id,edition:'offline-single-pc'});await load()}catch(e){error.value=e.response?.data?.message||'Approval failed.'}finally{busy.value=null}}
