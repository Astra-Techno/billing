...
<template>
  <div class="flex flex-col lg:flex-row h-full min-h-0 w-full overflow-hidden">
    ...
    <!-- Left Pane: List -->
    <div id="c3-left-panel" :class="{ 'hidden lg:flex': $route.name !== 'Quotes', 'w-full lg:w-[340px] border-r border-gray-200/60 flex flex-col shrink-0 bg-[#FAFAFA] transition-all duration-300 relative z-30 h-full': true }">
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex justify-between items-start mb-0.5">
                        <span class="text-[14px] font-bold truncate pr-2 tracking-tight transition-colors"
                              :class="$route.params.id == q.id ? 'text-indigo-600' : 'text-gray-900 group-hover:text-indigo-600'">
                              {{ q.client_name }}
                        </span>
    ...
    <div v-if="$route.name !== 'Quotes'" id="c3-right-view" class="flex-1 bg-[#F4F4F5] overflow-y-auto flex flex-col relative z-20 shadow-[-10px_0_20px_rgba(0,0,0,0.02)] custom-scrollbar">
